...
**Previous:** 2026-05-30 - **Phase 3 import/scan localization:** +73 strings in `strings_phase3.xml` (EN/DE); `stringResource()` on Import Ticket, History Photo Import, Live Camera, Scan tab, Scan Type sheet, OCR animation cards, Scan Result Quality card. Reused `import_ticket_*`, `settings_cancel`, `common_em_dash`. `scanTypeTitle`/`scanTypeSubtitle` composables for `BingoScanType`.

**Previous:** 2026-05-30 - **Phase 3 localization:** `strings_phase3.xml` (EN/DE) + `stringResource()` on Manual Entry, Ticket Detail, Ticket QR; `ManualEntryViewModel` / `TicketDetailViewModel` → `AndroidViewModel` for localized snackbars/dialogs. Reused `live_nav_manual_entry`, `common_*`, `history_*`, `settings_cancel`, `live_room_default_name`.

**Previous:** 2026-05-30 - **Phase 2 localization (complete):** EN/DE strings + plurals for Home, Profile, History list/detail, Live Rooms, Live Play, Jackpot (~130 strings, 7 plurals). `./gradlew :app:assembleDebug` OK.

**Previous:** 2026-05-30 - **Live Play + History Detail i18n:** `LivePlayScreen.kt`, `HistoryDetailScreen.kt` user-visible strings via `stringResource()`.

**Previous:** 2026-05-30 - **Auth + Settings i18n:** Added EN settings/validation strings to `values/strings.xml`; full DE translations in `values-de/strings.xml` for Login, Register, Forgot Password, Change Password, Settings. Screens use `stringResource()`. `./gradlew :app:assembleDebug` OK.

**Previous:** 2026-05-30 - **App language setting:** Moved language from My Account free-text to Settings APPEARANCE dropdown (English / Deutsch). `AppLanguage` model + `AppLanguageViewModel`; DataStore keys `app_language` / `app_language_user_set`; first-launch device locale default (de→Deutsch, else English); `AppCompatDelegate.setApplicationLocales` in `MainActivity`. `./gradlew :app:assembleDebug` OK.

**Previous:** 2026-05-30 - **Login wave/layout fix:** Split Login into fields column + compact bottom `AuthBottomWave` box (forgot/button/footer only). Moved `AppImeFormScrollBottomSpacer` outside wave box so IME scroll range does not stretch wave. `./gradlew :app:assembleDebug` OK.
...
- **Auth + Settings i18n (EN/DE):** `stringResource` on Login, Register, Forgot Password, Change Password, Settings; `values-de/strings.xml` for those screens only.
- **App language (Settings):** English / Deutsch dropdown in Settings APPEARANCE; device locale default on first launch only; persisted in DataStore; locale applied via AppCompat; My Account language field removed.
- **Scan type selection (Direct Scan):** `ScanTypeSelectionSheet` on Scan tab; `BingoScanType` nav arg on `bingoLiveCameraImport`; History/Jackpot default PLAY_PAPER.
