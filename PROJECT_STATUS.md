...
**Last update:** 2026-05-30 - **Auth + Settings i18n:** Added EN settings/validation strings to `values/strings.xml`; full DE translations in `values-de/strings.xml` for Login, Register, Forgot Password, Change Password, Settings. Screens use `stringResource()`. `./gradlew :app:assembleDebug` OK.
...
## Completed features`r`n`r`n- **Auth + Settings i18n (EN/DE):** `stringResource` on Login, Register, Forgot Password, Change Password, Settings; `values-de/strings.xml` for those screens only.
