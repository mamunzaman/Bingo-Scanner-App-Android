...
**Last update:** 2026-04-19 — Live play ⋮ menu: **Reset game** opens the existing reset confirm; on confirm, calls clear to 0 and play can continue (`LiveRoomTopBar` + `LivePlayScreen`). `./gradlew :app:assembleDebug` OK.
...
## Completed features`r`n`r`n- **Live ⋮ Reset game**: top bar overflow includes **Reset game** (same flow as existing Reset — confirm dialog, `RoomRepository.resetCalledNumbers`, un-archive room).
