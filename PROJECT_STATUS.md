...
**Last update:** 2026-05-30 - **App language setting:** Moved language from My Account free-text to Settings APPEARANCE dropdown (English / Deutsch). `AppLanguage` model + `AppLanguageViewModel`; DataStore keys `app_language` / `app_language_user_set`; first-launch device locale default (de→Deutsch, else English); `AppCompatDelegate.setApplicationLocales` in `MainActivity`. `./gradlew :app:assembleDebug` OK.
...
## Completed features`r`n`r`n- **App language (Settings):** English / Deutsch dropdown in Settings APPEARANCE; device locale default on first launch only; persisted in DataStore; locale applied via AppCompat; My Account language field removed.
